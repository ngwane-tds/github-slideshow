...
export class SubmitProjectService {
  ...
  public async submitProject(
    ...
    let payload: any;
    let endpoint = '';
    console.log('Project Submit -> ', projectTypeId);
    switch (projectTypeId) {
      ...
      case 10:
        ...
        payload = {
          ...
          ExistingConditionLanes:
            roadwayFieldsValue.existingConditionLanes,
        ...
        endpoint = `${endpoints.API_SUBMIT.ROADWAY}?id=${roadwayFieldsValue.id}&lang=${language}`;
        break;
      // Interchange
      case 20:
        ...
        endpoint = `${endpoints.API_SUBMIT.INTERCHANGE}?id=${payload.Id}&lang=${language}`;
        break;
      // Rail
      case 30:
        ...
        endpoint = `${endpoints.API_SUBMIT.RAILROAD}?id=${payload.Id}&lang=${language}`;
        break;
      // POE
      case 40:
        ...
        endpoint = `${endpoints.API_SUBMIT.PORT_OF_ENTRY}?id=${payload.Id}&lang=${language}`;
        break;
      ...
      case 50:
        ...
        endpoint = `${endpoints.API_SUBMIT.CROSS_BORDER}?id=${payload.Id}&lang=${language}`;
        break;
      ...
      case 60:
        ...
        endpoint = `${endpoints.API_SUBMIT.SHORT_TERM}?id=${payload.Id}&lang=${language}`;
        break;
      default:
        const err = new Error(
          `Project Type case not handled in SUBMIT function: ${projectTypeId}`
        );
        this.store.dispatch(new homeActions.ProjectSubmitError(err));
        throw err;
    }
    try {
      ...
      res.project = payload;
      if (res.isSubmitted) {
        res.project.Project.StatusId = ProjectStatusType.submittedId; // update status to submitted
        this.store.dispatch(new homeActions.GetProjects(language));
        this.store.dispatch(new homeActions.ProjectSubmitSuccess(res));
      } else {
        this.store.dispatch(new homeActions.ProjectSubmitError(res));
      }
      return res;
    } catch (err) {
      this.store.dispatch(new homeActions.ProjectSubmitError(err));
